...
void PrismView3DStatic::Serialize(CArchive& ar)
{
	const WORD sVersion = 1;
	if (ar.IsStoring())
	{
		ar << sVersion;
		ar << (BYTE)m_bMeshSet;
...
void PrismView3DStatic::SetMeshInternal()
{
	/* load into opengl */
	if (m_VAO)
	{
...
void PrismView3DStatic::SetTexelsPerUnit(float newVal)
{
	if (m_VAO)
	{
		m_Mesh.SetTexelsPerUnit(newVal);
		glDeleteTextures(1, &m_Texture);
		OpenGlGPULoadTexture(m_Mesh.GetTextureData(), m_Mesh.GetTextureWidth(), m_Mesh.GetTextureHeight(), &m_Texture);
		Invalidate(FALSE);
