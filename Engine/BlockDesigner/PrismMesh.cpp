...
IMPLEMENT_SERIAL(PrismMesh, CObject, 1)
...
void PrismMesh::SetTexelsPerUnit(float newVal)
{
	m_TexelsPerUnit = newVal;
...
void PrismMesh::Serialize(CArchive& ar)
{
	...
	if (ar.IsStoring())
	{
		...
		ar << m_Normals.size();
		for (const glm::vec3& norm : m_Normals)
		{
			ar << norm.x;
			ar << norm.y;
			ar << norm.z;
		...
		ar << m_BottomCapVertsStart;
		ar << m_BottomCapVertsEnd;

		ar << m_BottomCapIndsStart;
		ar << m_BottomCapIndsEnd;

		ar << m_TopCapVertsStart;
		ar << m_TopCapVertsEnd;

		ar << m_TopCapIndsStart;
		ar << m_TopCapIndsEnd;

		ar << m_SidesVertsStart;
		ar << m_SidesVertsEnd;
...
void PrismMesh::GenerateUVsAndTexture()
{
	...
	po.texelsPerUnit = m_TexelsPerUnit;
...
void PrismMesh::operator=(const PrismMesh& other)
{
	...
	m_BottomCapVertsStart = other.m_BottomCapVertsStart;
	m_BottomCapVertsEnd = other.m_BottomCapVertsEnd;

	m_BottomCapIndsStart = other.m_BottomCapIndsStart;
	m_BottomCapIndsEnd = other.m_BottomCapIndsEnd;

	m_TopCapVertsStart = other.m_TopCapVertsStart;
	m_TopCapVertsEnd = other.m_TopCapVertsEnd;

	m_TopCapIndsStart = other.m_TopCapIndsStart;
	m_TopCapIndsEnd = other.m_TopCapIndsEnd;

	m_SidesVertsStart = other.m_SidesVertsStart;
	m_SidesVertsEnd = other.m_SidesVertsEnd;

	m_SidesIndsStart = other.m_SidesIndsStart;
	m_SidesIndsEnd = other.m_SidesIndsEnd;
}
