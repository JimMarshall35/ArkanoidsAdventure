...
#include "WindowsVulkanBackendExports.h"
...
private:
    ...
    void createInstance() {
        VkApplicationInfo appInfo{};                              //optional but recommended as gives useful info the driver can use to optimize
    ...
    void createSwapChain()
    {
        ...
        */
        ...
        /*
            Make sure not to exceed max. 0 is a special value that means that there is no maximum
        */
        ...
        createInfo.preTransform = swapChainSupport.capabilities.currentTransform; // can apply transforms to the swap chain images, flip horizontal, rotate 90degrees, ect, but we don't want to so specify current
    ...
    void createGraphicsPipeline()
    {
        ...
        // which variables of the pipleines state be changed without recreating pipeline
        ...
        rasterizer.lineWidth = 1.0f;                     // width of drawn lines in fragments (pixels)
    ...
    void recreateSwapChain()
    {
        ...
        {
            // wait while width or height is zero (window is minimised)
            glfwGetFramebufferSize(window, &width, &height);
            glfwWaitEvents();
        }
...
#define MS_PER_UPDATE 20
    ...
    void mainLoop()
    {
        ...
        double previous = glfwGetTime();//getCurrentTime();
        double lag = 0.0;
        while ((!glfwWindowShouldClose(window)))
        {
